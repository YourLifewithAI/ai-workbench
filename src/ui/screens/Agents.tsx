        ...
        <div className="mt-4 space-y-2" role="alert">
          {q.data.errors.map((e) => (
            <Card key={e.id} className="border-l-4 border-l-red-600 dark:border-l-red-400">
              ...
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Fix the file, then <Button variant="link" size="inline" className="underline" onClick={() => reload.mutate()}>reload</Button>.</p>
            </Card>
          ))}
        </div>
      ) : null}
...
export function AgentDetail() {
  ...
  return (
    <section aria-labelledby="screen-title">
      ...
      <ScreenTitle className="mt-2">{q.data?.name ?? id}</ScreenTitle>
      {q.data ? (
        <>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <Card>
              <CardTitle>Instruction sections</CardTitle>
