...
export const api = {
  ...
  spend: (): Promise<SpendResponse> => apiFetch('/spend').then((r) => r.json() as Promise<SpendResponse>),
  estimate: (body: EstimateRequest): Promise<EstimateResponse> =>
    apiFetch('/runs/estimate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then((r) => r.json() as Promise<EstimateResponse>),
  ...
  /** A project's space (D-69): read with the version a save must pin; saved hash-pinned (409 when the file moved). */
  space: (slug: string): Promise<ProjectSpaceResponse> => apiFetch(`/projects/${encodeURIComponent(slug)}/space`).then((r) => r.json() as Promise<ProjectSpaceResponse>),
  saveSpace: (slug: string, body: SaveProjectSpaceRequest): Promise<ProjectSpaceResponse> =>
    apiFetch(`/projects/${encodeURIComponent(slug)}/space`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then((r) => r.json() as Promise<ProjectSpaceResponse>),
  ...
  work: (state = 'open'): Promise<WorkItem[]> => apiFetch(`/work?state=${encodeURIComponent(state)}`).then((r) => r.json() as Promise<WorkListResponse>).then((b) => b.items),
  ...
  findings: (): Promise<PermissionFinding[]> => apiFetch('/permissions/findings').then((r) => r.json() as Promise<PermissionFindingsResponse>).then((b) => b.findings),
  decideFinding: (id: string, decision: 'apply' | 'dismiss'): Promise<PermissionFinding> =>
    apiFetch(`/permissions/findings/${encodeURIComponent(id)}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ decision }) }).then((r) => r.json() as Promise<PermissionFinding>),
