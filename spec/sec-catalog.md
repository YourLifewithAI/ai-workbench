...
| SEC-38 | A project's tool ceiling never widens: for every (agent, tool, project) the broker's decision with the ceiling is at most as permissive as without it; a ceiling entry for a tool the agent has no grant for changes nothing; no `project.json` key can lift an approval or widen `maxPermissions`, and a project's memory list only removes scopes (D-69) | 18 |
| SEC-41 | The owner's page is an instruction only while a person wrote its latest version: a version a run filed goes into every prompt fenced, with a `profile-fenced` event, and moves nothing in `promptVersion`; `owner.profile` is set by an authenticated human request and no tool can point it anywhere; a run cannot make its own draft the page (D-74) | 23 |
| SEC-42 | The orchestrator sees facts, not content: `runs.facts` and `agents.read` have `maxPermissions` that admit no path, host or credential, and a phrase planted in a run's task, its output, a document and a tool's arguments appears nowhere in their results; `agents.read` returns no grant; `runs.rate` writes `scores` only — `ratings` is untouched by any tool — and no score of any kind reaches model or agent selection (D-73, D-06, D-50) | 23 |
| SEC-43 | Taint flows up at return: a run that delegates to a child which read outside the workspace is itself external-tainted when the child returns, so what it then remembers is `untrusted`; a delegation that failed carries nothing; the child's taint rides on the tool result's `meta`, never in the output the model reads (D-17, D-12) | 23 |
| SEC-44 | Ledger text is content: a work item written by an external-tainted run is `untrusted` and shown so; no item's title, detail or answer reaches an instruction section of any prompt; `work.file`, `work.list`, `work.update` and `owner.ask` have `maxPermissions` that admit no path, host or credential; an item is filed once per key and refreshed after, never duplicated (D-75) | 24 |
| SEC-46 | A detached child cannot escape budget: `wait: false` charges the carved budget to the parent as a reservation at dispatch (unsized, half of what the parent has left, so the parent can still finish its own turn), a model call in any descendant run counts against every ancestor agent's own daily and monthly caps — through `runs.parent_run_id`, and through `run_steps.agent_id` and `runs.parent_step_id` when the ancestor was a step inside a workflow run, whose budget carries its agent's caps — depth is refused past three, and a child of a finished parent runs to completion inside its carve (D-76) | 24 |
